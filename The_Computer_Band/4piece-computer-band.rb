...
# The Start of the Menu system

puts " The Computer Band Music Composer \n"
...
song.tracks << (guitar = TimedTrack.new(4, song ))

melody.instrument = 34 # 
background.instrument = 19 # Church organ
drumz.instrument = 99
chord.instrument = 01
guitar.instrument = 23

# names the Tracks 
melody.name = "melody"  
background.name = "melody 2"
drumz.name = "Drumz"
chord.name = "Chords"
guitar.name = "Guitar"
...
(16*16).times do |i|
  ...
  drumz.add_major_13th(note, rand(127),note_length[rand(note_length.size)])
  background.add_notes(note, rand(127),note_length[rand(note_length.size)])
  chord.add_major(note, rand(127),note_length[rand(note_length.size)]) if i % 4 == 0
  guitar.power_chord(note,rand(127),note_length[rand(note_length.size)]) if i % 4 == 0
